...
首版故意只使用已有图表指标: MA, MACD, RSI, KDJ, momentum/ROC, 量比,
ATR 和 BOLL. 新增指标应先加入指标流水线, 再按版本更新本模块.
"""
...
TECHNICAL_SCORE_VERSION = "technical-score-v1"
...
# 方向分的顶层权重; 波动和成交活跃度只参与风险/置信度, 不参与方向总分.
_DIRECTION_WEIGHTS = {
    "trend": 0.40,
    "momentum": 0.35,
    "volume_price": 0.25,
...
def _volume_price_score(records: list[dict], index: int) -> tuple[float | None, float]:
    ...
    return _clamp(50.0 + 50.0 * (1.0 if change > 0 else -1.0) * conviction), 1.0
...
def _score_records(records: list[dict]) -> list[dict[str, float | bool | None]]:
    ...
    for index in range(len(records)):
        ...
        direction_parts = [(score, weight) for score, weight, _ in dimensions if score is not None]
        direction_score, _ = _weighted_mean(direction_parts)
        coverage = sum(weight * component_coverage for _, weight, component_coverage in dimensions)
        valid_dimensions = sum(score is not None for score, _, _ in dimensions)
        ...
        activity_factor = 0.60 + 0.40 * (activity / 100.0) if activity is not None else 0.75
        risk_factor = 1.0 if volatility is not None and volatility <= 60 else (
            max(0.80, 1.0 - (volatility - 60.0) / 200.0) if volatility is not None else 0.90
        )
        confidence = _clamp(100.0 * coverage * activity_factor * risk_factor)
        result.append({
