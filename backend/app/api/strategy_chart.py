...
@router.get("/signal-markers")
def get_signal_chart_markers(
    request: Request,
    symbol: str = Query(..., description="标的代码"),
    asset_type: Literal["stock", "etf"] = Query(..., description="资产类型"),
    timeframe: Literal["1d", "1w", "30m", "1mo"] = Query("1d", description="K线周期"),
    start_date: date = Query(..., description="展示起始日期"),
    end_date: date = Query(..., description="展示截止日期"),
    signal_ids: str = Query(..., description="逗号分隔的信号 ID"),
    ...
    selected_ids = list(dict.fromkeys(
        item.strip() for item in signal_ids.split(",") if item.strip()
    ))
    if not selected_ids:
        raise HTTPException(status_code=422, detail="至少选择一个信号")
    if len(selected_ids) > 32:
        raise HTTPException(status_code=422, detail="单次最多选择 32 个信号")
    ...
    try:
        markers = StrategyChartService(
            repo,
            getattr(request.app.state, "strategy_engine", None),
            getattr(request.app.state, "chart_data_service", None),
        ).signal_markers(
            symbol=symbol,
