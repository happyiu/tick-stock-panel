...
function Metric({ label, value, tone = 'neutral' }: { label: string; value: string; tone?: Tone }) {
  return <div className="min-w-0"><div className="text-[9px] text-muted">{label}</div><div className={`truncate font-mono text-[10px] tabular-nums ${toneClass(tone)}`} title={value}>{value}</div></div>
}

function Section({ title, badge, children }: { title: string; badge?: string; children: React.ReactNode }) {
  return <section className="rounded-card border border-border bg-surface/70 p-2.5 shadow-sm"><div className="mb-2 flex items-center justify-between gap-2"><div className="text-[11px] font-medium text-foreground">{title}</div>{badge && <span className="rounded border border-border bg-elevated px-1.5 py-0.5 text-[9px] text-secondary">{badge}</span>}</div>{children}</section>
}

function PositionBar({ analysis }: { analysis: ChanlunAnalysis }) {
  const center = analysis.latestCenter
  if (!center || analysis.currentPrice == null) return <div className="text-[9px] text-muted">等待有效中枢</div>
  const width = center.zg - center.zd
  if (width <= 0) return <div className="text-[9px] text-muted">ZG 与 ZD 重合，属于零宽重叠，无法计算中枢位置。</div>
...
function MultiPeriodTable({ rows }: { rows: ChanlunMultiPeriodItem[] }) {
  ...
  })}</tbody></table></div><div className="mt-2 rounded bg-elevated/60 px-2 py-1 text-[9px] text-secondary">{summary}</div></>
}
  ...
  return <section>
    <div className={`flex w-full items-start justify-between gap-2 px-2.5 py-2 ${collapsed ? 'border-b border-border/70' : ''}`}>
      <button type="button" onClick={onToggleCollapsed} className="flex min-w-0 flex-1 items-start gap-1.5 text-left" aria-expanded={!collapsed}>
        ...
        <span className="min-w-0"><span className="flex flex-wrap items-center gap-1.5"><span className="text-xs font-medium text-foreground">缠论</span><span className="rounded bg-[#8B5CF6]/10 px-1.5 py-0.5 text-[9px] text-[#A78BFA]">结构近似 v{analysis.ruleVersion}</span><span className="rounded bg-elevated px-1.5 py-0.5 font-mono text-[9px] text-secondary">{PERIOD_LABELS[period]}</span></span>{!collapsed && <span className="mt-0.5 block truncate text-[10px] text-muted" title={analysis.approximationLoss}>{analysis.approximationLoss}</span>}</span>
      </button>
      {!collapsed && onSourceChange && <div className="flex rounded border border-border bg-base p-0.5 text-[9px]">{(['stroke', 'segment'] as const).map(value => <button key={value} type="button" onClick={() => onSourceChange(value)} className={`rounded px-1.5 py-0.5 ${source === value ? 'bg-[#8B5CF6]/20 text-[#A78BFA]' : 'text-muted'}`}>{value === 'stroke' ? '笔中枢' : '线段中枢'}</button>)}</div>}
    </div>
