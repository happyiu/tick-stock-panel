...
export function ChartDataNotice({ status, hideDate = false }: { status?: ChartDataStatus; hideDate?: boolean }) {
  ...
  return (
    <div className="px-1 py-1 text-[11px] text-muted break-words" role="status">
      {status.stale
        ...
        : `行情获取于 ${fetched}`}
      {!hideDate && status.data_through && ` · 数据截至 ${status.data_through.slice(0, 16)}`}
